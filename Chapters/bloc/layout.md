...
bloc element, you'll need to combine them together.
...
of them are layout together. Each element can be build and inspected
individually, which is good for development purpose. If you want to build a
full graphical interface, you'll need to combine element with each others, or
at at least add it to a *space*.
...
widget in a single method, it encourage you to build small element and combine them
using various layout strategy. The layout property define the visual agencement
of the element and its children. It defines the position and size of child and
grand-child elements within the parent. It can also have an impact on the size
of the parent element.

When defining layout, 2 parts must be combined and play together: *parent* and
*children* elements.

* Parent define which layout strategy apply to their children
* children specify which constraint they apply, which could impact their position and
their size.

Note that if you don't specify which layout your parent element will use, it'll
default to *BlBasicLayout* strategy.

### element combination

Element can be combined with the **addChild:** method. You can add multiple
...
to manage the element composing your widget.

### space around elements

First, before jumping on the position of each element, you can already specify
how close your element will be from each other, with those 2 properties
...
Element are disposed in a grid.
